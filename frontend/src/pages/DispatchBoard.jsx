...
const MAP_MARKERS = [
  { id: 1, label: "Providencia",     lat: -33.4320, lng: -70.6093, status: "En camino",   hex: "#f97316" },
  { id: 2, label: "Las Condes",      lat: -33.4103, lng: -70.5684, status: "En camino",   hex: "#f97316" },
  { id: 3, label: "Ñuñoa",          lat: -33.4569, lng: -70.5990, status: "Completada",  hex: "#34d399" },
  { id: 4, label: "Santiago Centro", lat: -33.4489, lng: -70.6693, status: "Programada",  hex: "#60a5fa" },
];
...
// ═══════════════════════════════════════════════════════════════════════════════
function ActiveZonesCard() {
  return (
    <div className="rounded-xl border border-border bg-card p-5 flex flex-col gap-4">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <div>
            <h2 className="text-sm font-semibold text-foreground">Zonas activas</h2>
            <p className="text-xs text-muted-foreground">Distribución geográfica de OT</p>
          </div>
          <span className="h-1.5 w-1.5 rounded-full bg-primary animate-pulse shrink-0" aria-label="En vivo" />
        </div>
        <button
          type="button"
      ...
      <div className="relative rounded-lg overflow-hidden border border-border" style={{ height: "200px" }}>
        <MapContainer
        ...
        >
          ...
          {MAP_MARKERS.map((m) => (
            <Marker key={m.id} position={[m.lat, m.lng]} icon={makeIcon(m.hex)}>
              <Popup>
                <div style={{ fontSize: "12px", lineHeight: "1.5", minWidth: "100px" }}>
...
const UPCOMING_VISITS = [
  { time: "14:00", service: "Instalación fibra óptica",        client: "Sr. Ramírez — Providencia",   status: "En camino",   statusStyle: "bg-orange-500/15 text-orange-400 ring-1 ring-orange-500/25" },
  { time: "14:30", service: "Cambio de SIM corporativa",       client: "Entel HQ — Santiago Centro",  status: "Programada",  statusStyle: "bg-blue-500/15 text-blue-400 ring-1 ring-blue-500/25" },
  { time: "15:15", service: "Diagnóstico router empresarial",  client: "Banco Estado — Las Condes",   status: "Programada",  statusStyle: "bg-blue-500/15 text-blue-400 ring-1 ring-blue-500/25" },
  { time: "16:00", service: "Revisión antena 4G",              client: "Torre Entel — Las Condes",    status: "Reagendada",  statusStyle: "bg-rose-500/15 text-rose-400 ring-1 ring-rose-500/25" },
  { time: "17:30", service: "Activación Magic Link",           client: "PYME Clientes — Ñuñoa",      status: "Programada",  statusStyle: "bg-blue-500/15 text-blue-400 ring-1 ring-blue-500/25" },
];
...
function DonutChartCard() {
  ...
  return (
    <div className="rounded-xl border border-border bg-card p-5 flex flex-col gap-4">
      <div>
        <h2 className="text-sm font-semibold text-foreground">Estado de visitas</h2>
        <p className="text-xs text-muted-foreground">Distribución del estado actual</p>
      </div>
      ...
      <div className="flex items-center justify-center py-2">
        <div className="relative">
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center">
            ...
            <span className="text-[10px] text-muted-foreground">visitas hoy</span>
          </div>
        </div>
      </div>
...
const ACTIVITY_LOG = [
  { id: "#OT-08491", technician: "Juan Rivas",      action: "Marcó visita como completada",        time: "13:52" },
  { id: "#OT-08495", technician: "Elena Kravets",   action: "Inició desplazamiento hacia el lugar", time: "13:47" },
  { id: "#OT-08492", technician: "Luis Pinto",      action: "Aceptó la OT asignada",               time: "13:30" },
  { id: "#OT-08496", technician: "Elena Kravets",   action: "Actualizó estado a En Tránsito",      time: "13:15" },
  { id: "#OT-08499", technician: "Luis Pinto",      action: "Marcó visita como completada",        time: "12:58" },
  { id: "#OT-08493", technician: "María Kovalenko", action: "Aceptó la OT asignada",               time: "12:40" },
];
  ...
  return (
    <div className="rounded-xl border border-border bg-card overflow-hidden">
      ...
      <div className="flex items-center justify-between px-5 py-4 border-b border-gray-800">
        <div>
          <h2 className="text-sm font-semibold text-foreground">Registro de actividad reciente</h2>
          <p className="text-xs text-muted-foreground">Últimas acciones registradas por los técnicos</p>
        </div>
        ...
        <table className="w-full text-left">
          <thead>
            <tr className="border-b border-gray-800">
              <th className="px-5 py-3 text-[10px] font-bold uppercase tracking-widest text-gray-500 w-28">ID Orden</th>
              <th className="px-5 py-3 text-[10px] font-bold uppercase tracking-widest text-gray-500">Técnico</th>
              <th className="px-5 py-3 text-[10px] font-bold uppercase tracking-widest text-gray-500">Acción</th>
              <th className="px-5 py-3 text-[10px] font-bold uppercase tracking-widest text-gray-500 text-right w-20">Hora</th>
            </tr>
          </thead>
  ...
  return (
    <>
      ...
      <div className="flex flex-col gap-6">
        ...
        {/* ── Tabla de actividad — ancho 100% (abraza sidebar + contenido central) ── */}
        <ActivityTable />
